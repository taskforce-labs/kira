...
3. Mark task done: `kira slice task current toggle` (or `kira slice task toggle <work-item-id> <task-id>`). Toggle does **not** commit by default; to commit, either run `kira slice commit generate | git commit -F -` or use `kira slice commit current`. To commit the toggle in the same step, use `--commit`/`-c` (e.g. `kira slice task current toggle --commit`).
