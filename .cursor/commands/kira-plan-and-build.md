...
1. **Get current slice/tasks:** `kira slice current` (no argument when work item is clear from context). To specify the work item, pass one argument: `current` or `<work-item-id>` — e.g. `kira slice current 047-foo`. Use `--output json` for machine-readable output. Same for `kira slice task current`.
