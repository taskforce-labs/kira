...
2. **Implementation Loop**
  ...
  - Commit: use `kira slice commit` (or `git commit` after staging) for the slice/work item. Use `kira slice task done current --commit` to commit the work-item change in the same step as marking done.
